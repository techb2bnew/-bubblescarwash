-- Capture the customer's car number/plate on a booking, so it can be shown
-- alongside every other booking detail (admin tables, emails, calendar
-- events) — useful once a business has several vehicles booked in the same
-- window and needs to tell them apart at a glance.

alter table bookings add column if not exists car_number text;

-- create_booking: same as before plus p_car_number (new trailing default
-- parameter, so existing positional call sites keep working).
drop function if exists create_booking(uuid, text, date, time, text, text, text, uuid[], text, text);
...
begin
  ...
  insert into bookings (
    ...
    gift_card_id, gift_card_discount, discount_id, discount_amount, car_number
  ) values (
...
grant execute on function create_booking(uuid, text, date, time, text, text, text, uuid[], text, text, text)
  to anon, authenticated;

-- get_booking_payment_status: also return car_number for the confirmation page.
drop function if exists get_booking_payment_status(uuid);
...
as $$
  select b.status, b.payment_status, b.customer_name, b.booking_date,
         b.booking_time, b.price, s.name, b.car_number
  from bookings b
...
-- mark_booking_paid: also return car_number so the Stripe webhook can pass
-- it straight to onBookingCreated without a second query.
drop function if exists mark_booking_paid(text, text, text, text, text);
...
begin
  ...
  return query
    select b.id, b.service_id, b.customer_name, b.customer_phone, b.customer_email,
           b.booking_date, b.booking_time, b.price, b.car_number
    from bookings b
    where b.id = v_id;
end;
$$;
